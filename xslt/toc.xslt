...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template match="/index">
        ...
        <a href="about.html"
                target="main">About</a> |  
        <a
                href="http://code.google.com/p/kettle-cookbook/" target="_blank">Project</a>
        </div>
        <div id="tab-strip">
            <!--
                    ...
                    <xsl:when test="$connections">
                        <ul>
                            <xsl:for-each select="$connections">
                                <xsl:sort
                                    select="translate(@name, $upper-case-alphabet, $lower-case-alphabet)"/>
                                <xsl:variable name="name" select="@name"/>
                                <li class="item item-connection">
                                    <a target="main" class="item item-connection">
                                        <xsl:attribute name="href">
                                            <xsl:value-of
                                                select="concat('connection-', $name, '.html')"/>
                                        </xsl:attribute>
    ...
    <xsl:template match="file">
        ...
        <li>
            <xsl:attribute name="class">item item-<xsl:value-of select="@extension"
                /></xsl:attribute>
            <a target="main">
                <xsl:attribute name="class">item item-<xsl:value-of select="@extension"
                    /></xsl:attribute>
            ...
            <div class="folder">
                <div class="folder-head">
                    ...
                    <span class="folder-label">
                        <xsl:value-of select="$folder/@short_name"/>
                    </span>
                </div>
                <div class="folder-body">
